...
% load grf batch
grf_batch = load('F:/Ultrasound/GRF/grf_2.3_size128_num200.mat','-mat');
batch_size = size(grf_batch.grf,1);

tof_array = zeros(num_sensor_points,num_sensor_points);
...
for grf_id = 1:100
    ...
    mask = distance<0.5;
    eroded_mask = imerode(mask, ones(5, 5));
    grf = squeeze(grf_batch.grf(grf_id,:,:));
    sound_speed = 1500+grf/max(max(abs(grf)))*50;
    threshold = 1510;
    sound_speed(sound_speed>=1.00*mean(mean(sound_speed))) = 1550;
    sound_speed(sound_speed<threshold) = 1450;
    sound_speed(mask==0) = 1500;
    sound_speed((mask-eroded_mask)==1) = 1580;
    medium.sound_speed = sound_speed;
    figure('visible','off');
    set(gcf,'position',[1080,720,700,600])
    fig1 = imagesc(sound_speed, [min(min(sound_speed)), max(max(sound_speed))]);
    colormap('jet');
    ylabel('y');
    xlabel('x');
    colorbar;
    save(sprintf('F:/Ultrasound/GRF/mats/sound_speed/grf%d_sound_speed.mat', grf_id+300),'sound_speed','-mat');
    saveas(fig1, sprintf('F:/Ultrasound/GRF/figures/grf%d_sound_speed.png', grf_id+300));
    
    for pressure_pos_id = 1:num_pressure_point
        disp([grf_id+300 pressure_pos_id]);
        ...
        sensor_data = kspaceFirstOrder2D(kgrid, medium, source, sensor, ...
            grf_id+300, pressure_pos_id, 'PlotLayout', false, 'PlotPML', false, 'PlotSim', false);%'SaveLayoutWoDisp', true, 
...
%             figure;plot(linspace(0,1,1000), [sensor_data(pressure_pos_id,:) awgn(sensor_data(pressure_pos_id,:),20)]);
        ...
        save(sprintf('F:/Ultrasound/GRF/mats/distribution_pair/grf%d_pos%d_distribution_pair.mat', grf_id+300, pressure_pos_id),'simulation_data','-mat');
    end

    tof_water = load('F:/Ultrasound/GRF/water_tof.mat');

    tof_array = tof_array-tof_water.tof_array;
    figure('visible','off');
    ...
    colorbar;
    saveas(fig2, sprintf('F:/Ultrasound/GRF/figures/grf%d_tof.png', grf_id+300));
    save(sprintf('F:/Ultrasound/GRF/mats/tof/grf%d_tof.mat', grf_id+300),'tof_array','-mat');
end
